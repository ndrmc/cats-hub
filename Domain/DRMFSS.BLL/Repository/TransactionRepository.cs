   ...
   public partial class TransactionRepository : ITransactionRepository
    {
        ...
        public List<Account> GetActiveAccountsForLedger(int LedgerID)
        {
            var accounts = (from v in db.Transactions
                            ...
                            select v.Account).Distinct().OrderBy(o => o.EntityType).ToList();
            return accounts;
        }
        ...
        public decimal GetTotalReceiptAllocation(int siNumber,int commodityId , int hubId)
        {
            ...
            var allocationSum = (from v in db.Transactions
                                 where v.ShippingInstructionID == siNumber
                                       && v.HubID == hubId
                                       && (v.ParentCommodityID == commodityId || v.CommodityID == commodityId)
                                       && v.LedgerID == Ledger.Constants.GOODS_PROMISSED_GIFT_CERTIFICATE_COMMITED
                                       && v.QuantityInMT > 0
       ...
       public decimal GetCommodityBalanceForStore(int storeId, int parentCommodityId, int si, int project)
       {
           var balance = (from v in db.Transactions
                             where
                                 v.StoreID == storeId && v.ParentCommodityID == parentCommodityId &&
       ...
       public decimal GetCommodityBalanceForStack(int storeId, int stack , int parentCommodityId, int si, int project)
       {
           var balance = (from v in db.Transactions
                              where
                                  v.StoreID == storeId && v.ParentCommodityID == parentCommodityId &&
                                  v.ShippingInstructionID == si && v.ProjectCodeID == project && v.Stack == stack && v.LedgerID == Ledger.Constants.GOODS_ON_HAND_UNCOMMITED
                              select v.QuantityInMT);
           if (balance.Any())
           {
               return balance.Sum();
        ...
        public void SaveReceiptTransaction(ReceiveViewModel receiveModels, UserProfile user)
        {
            ...
            foreach (ReceiveDetailViewModel c in receiveModels.ReceiveDetails)
            {
                ...
                transaction.ParentCommodityID = repository.Commodity.FindById(c.CommodityID).ParentID ?? c.CommodityID;
                ...
                transaction.AccountID = repository.Account.GetAccountIDWithCreate(Account.Constants.HUB, receive.HubID);
                transaction.ShippingInstructionID = repository.ShippingInstruction.GetSINumberIdWithCreate(receiveModels.SINumber).ShippingInstructionID;
                ...
                if (receiveModels.CommoditySourceID == BLL.CommoditySource.Constants.DONATION || receiveModels.CommoditySourceID == BLL.CommoditySource.Constants.LOCALPURCHASE)
                {
                    ...
                    transaction2.AccountID = repository.Account.GetAccountIDWithCreate(Account.Constants.DONOR,receive.ResponsibleDonorID.Value);
                }
                else if (receiveModels.CommoditySourceID == BLL.CommoditySource.Constants.REPAYMENT)
                {
                    ...
                    transaction2.AccountID = repository.Account.GetAccountIDWithCreate(Account.Constants.HUB, receiveModels.SourceHubID.Value);
                }
                else
                {
                ...
                transaction2.ShippingInstructionID = repository.ShippingInstruction.GetSINumberIdWithCreate(receiveModels.SINumber).ShippingInstructionID;
